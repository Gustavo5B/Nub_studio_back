...
const isValidCode = (codigo) => /^\d{6}$/.test(codigo);
...
const sanitizePassword = (password) => {
  ...
  const maliciousPatterns = [
    ...
    /<iframe/i, /eval\(/i, /alert\(/i, /onload=/i, /<img/i,
    /on\w+\s*=/i, /data:/i, /vbscript:/i, /expression\(/i, /url\(/i
  ];

  for (const pattern of maliciousPatterns) {
    if (pattern.test(password)) throw new Error('La contrasena contiene caracteres no permitidos');
  }
...
const validatePasswordStrength = (password) => {
  ...
  if (password.length > 128) errors.push('La contrasena es demasiado larga (maximo 128 caracteres)');
  ...
  if (!/[@$!%*?&#._-]/.test(password)) errors.push('Debe contener al menos un caracter especial (@$!%*?&#._-)');
  ...
  if (/(.)\1{3,}/.test(password))
    errors.push('La contrasena no puede tener mas de 3 caracteres repetidos consecutivos');
  ...
  try {
    ...
    if (user.bloqueado_recuperacion_hasta) {
      ...
      if (ahora < desbloqueo) {
        ...
        const horaDesbloqueo = desbloqueo.toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' });

        logger.warn(`Recuperacion bloqueada: usuario ${user.id_usuario}, ${minutosRestantes} min restantes`);
    ...
    try {
      ...
      logger.error(`Error al enviar email de recuperacion: ${emailError.message}`);
    }
...
export const validateRecoveryCode = async (req, res) => {
  try {
    ...
    if (!isValidCode(codigo))
      return res.status(400).json({ message: "El codigo debe ser de 6 digitos" });
    ...
    if (result.rows.length === 0) {
      ...
      return res.status(401).json({ valid: false, message: "Codigo invalido o expirado" });
    }
...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (isSamePassword) {
      await client.query('ROLLBACK');
      return res.status(400).json({ message: "La nueva contrasena no puede ser igual a la anterior" });
    }
